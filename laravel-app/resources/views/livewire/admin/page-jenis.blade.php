                            ...
                            <tbody>
                                @foreach ($jenis as $item)
                                    <tr class="border-b border-black">
                                        <x-td><img src="{{ asset('upload/jenis/' . $item->gambar) }}" alt=""
                                                srcset="" width="100"></x-td>
                                        <x-td>{{ $item->nama_jenis }}</x-td>
                                        <x-td>
                                            <x-jet-button wire:click="tambahKatalog({{ $item->id }})">Katalog
                                            </x-jet-button>
                                        </x-td>
                                        <x-td>
                                            <x-jet-secondary-button wire:click='editJenisModal({{ $item->id }})'>
                                                Edit</x-jet-secondary-button> |
                                            <x-jet-danger-button wire:click='hapusJenis({{ $item->id }})'>Hapus
                                            </x-jet-danger-button>
                                        </x-td>
                                    </tr>
                                @endforeach
                            ...
                            <tbody>
                                @foreach ((object) $dataKatalog as $item)
                                    <tr>
                                        ...
                                        <x-td>
                                            <x-jet-danger-button wire:click="hapusKatalog({{ $item->id }})">Hapus
                                            </x-jet-danger-button>
                                        </x-td>
                                    </tr>
                                @endforeach
        ...
        <x-slot name='content' class="bg-white">
            <form class="relative w-full py-5 px-4 max-w-full flex-grow flex-1 bg-white">
                ...
                @elseif($editJenis == true)
                    <x-jet-button type='button' wire:click='editJenis({{ $itemID }})'>
                        EDIT
                    </x-jet-button>
                @endif
            </form>
