        ...
        <div class="container mx-auto px-4 col-span-4">
            ...
            <div class="grid grid-cols-3 gap-2 -mx-4 wow fadeInDown">
                @foreach ($produk as $item)
                    @if ($item->stock <=0 )
                        <div class="bg-slate-800 w-48 relative shadow overflow-hidden  cursor-not-allowed">
                    @endif
                        @if ($item->diskon->count() > 0)
                            @foreach ($item->diskon as $diskon)
                                <div class="absolute font-bold text-white px-2 py-1 bg-slate-600 rounded-r-sm shadow-lg shadow-white">
                        ...
                        <div class="pt-3 pb-4 px-2 group-hover:bg-gray-900">
                            ...
                            <div class="text-sm overflow-hidden h-8 leading-4 text-ellipsis line-through">{{ $item->nama_barang }} </div>
                            @else
