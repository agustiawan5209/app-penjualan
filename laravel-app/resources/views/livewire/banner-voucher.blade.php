<div class="relative top-20 shadow-lg text-white  w-full z-[1000]">
    @include('sweetalert::alert')
    @if ($UsesUserVoucher->count() > 0)
        <div class="sm:px-4 md:px-10 lg:px-14">
            @foreach ($UsesUserVoucher as $item)
                @if ($item->status == 1)
                        ...
                        <div class="max-w-screen-xl mx-auto py-2 px-3 sm:px-6 lg:px-8">
                            <div class="flex items-center justify-between flex-wrap">
                                ...
                                <div class="shrink-0 sm:ml-3 flex flex-wrap md:flex-nowrap items-center justify-around">
                                    <span class="text-white text-sm mr-3">Tanggal Kadaluarsa :
                                        {{ $item->tgl_kadaluarsa }} / Jam : {{ $item->waktu }}</span>
                                    {{-- <span class="text-white text-sm mr-3">Kadaluarsa {{ $carbon_date->diffInHours($item->waktu) }}</span> --}}

                                    <button type="button"
                                        class="-mr-1 flex p-2 rounded-md focus:outline-white cursor-pointer border border-white sm:-mr-2 transition text-white"
                                        :class="{
